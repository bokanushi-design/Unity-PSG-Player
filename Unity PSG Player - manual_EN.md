...
**PSG Player** stores MML—which represents musical notation as a string—in the **mmlString variable**, but to actually generate sound, it must be converted into sequence data in a List array, the **seqList variable**.  

Conversion from MML to sequence data is performed by the **MML Decoder**.  
The MML Decoder parses MML one character at a time, converts it into event data consisting of “Command, Parameter, Length,” and returns it as a sequence of data.  
