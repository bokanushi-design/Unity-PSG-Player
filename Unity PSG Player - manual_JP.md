...
2. 適当なゲームオブジェクトにMultiChannelControllerスクリプトをアタッチします。  
3. インスペクターからMultiChannelControllerの[psgPlayers](./Unity%20PSG%20Player%20-%20Script%20refernce_JP.md#psgplayers)に、設置したPSG Playerを割り当てます。  
![fig05](./img/fig05.png)

4. MultiChannelControllerの[multiChMMLString](./Unity%20PSG%20Player%20-%20Script%20refernce_JP.md#multichmmlstring)変数にMMLを入れて、[SplitMML()](./Unity%20PSG%20Player%20-%20Script%20refernce_JP.md#splitmml)で各チャンネルにMMLを分配し、[PlayAllChannels()](./Unity%20PSG%20Player%20-%20Script%20refernce_JP.md#playallchannels)で再生されます。  
![fig06](./img/fig06.png)
...
* MMLを分割する「**MultiChannelController**」

で構成されています。  

単音で使用する場合はMultiChannelControllerは必要ありません。  
...
## マルチチャンネル時のチャンネル間同期について

`v0.9.8beta`

マルチチャンネルで再生する場合、各チャンネルは独立して時間を計算します。  
倍精度で計算するので誤差は起きにくいですが、ループで繰り返すと積算して気になるレベルまでズレる可能性があります。  
そこで、チャンネル間の位置を合わせる仕組みを、Channel Sync Markerとして用意しました。  

### Channel Sync の原理

各チャンネルの同じ（であって欲しい）タイミングにChannel Sync Markerを置きます。  

![ChannelSync1](./img/ChannelSync01.drawio.svg)

&nbsp;  
先に到達したMarkerの位置を記録します。

![ChannelSync2](./img/ChannelSync02.drawio.svg)

&nbsp;  
遅れて到達したMarkerは、登録された位置で到達済みとみなして、次の音符・休符の長さを調整します。

![ChannelSync3](./img/ChannelSync03.drawio.svg)

&nbsp;  
結果として、Marker直後の音符・休符の長さはズレた分だけ短くなり、イベント終了後の位置が同じになります。

![ChannelSync4](./img/ChannelSync04.drawio.svg)

### 注意点

上記の仕組みから、Channel Sync Markerの位置で同期するのではなく、**Marker直後の音符・休符の後で同期**します。  
ループさせる場合、シーケンスの一番最後にMarkerを置いても、ズレを調整する音符・休符が無いので効果が出ません。  
1小節前や1拍前などにMarkerを置いてください。  

チャンネル間のズレは、0.5秒以内で許容されます。  
これは、リピート内で同じMarkerが呼び出されたときに、意図しない動作とならないための対策です。  

Markerはシーケンス内に複数置くことができますが、各チャンネルでの出現順に対応します。  
例えば、Aチャンネルで1,2,3小節の頭、Bチャンネルで1,3小節の頭にMarkerを置いた場合、1小節目では同期しますが、Aチャンネルの2小節目はBチャンネルの3小節目に対応するので、大抵は前述の0.5秒の許容範囲から外れるため、ちゃんと動作しません。
基本的には、全チャンネルで同じタイミングにMarkerを置くようにしてください。  
