...
public class MultiChannelController : MonoBehaviour
{
    ...
    private int allChSampleRate = ConstValue.DEFAULT_SAMPLE_RATE;

    private List<double> chSyncPositionList = new();
    private float chSyncToleranceTime = ConstValue.DEFAULT_TOLERANCE_TIME;
    ...
    {
        int _id = 0;
        foreach(var pp in psgPlayers)
        {
            pp.multiChannelController = this;
            pp.channelId = _id;
            _id++;
        }
    }
    ...
    /// <summary>
    /// A manager that corrects timing between channels.
    /// Called by each PSGPlayer and returns a correction value.
    /// </summary>
    /// <param name="channelId">Channel ID of the called PSG Player</param>
    /// <param name="syncIndex">Order of the Channel Sync Markers</param>
    /// <param name="eventPosition">Event position of the Channel Sync Marker</param>
    /// <returns>New event position to correct</returns>
    public double ChannelSyncManager(int channelId, int syncIndex, double eventPosition)
    {
        string logText;
        double result = eventPosition;
        if (chSyncPositionList.Count < syncIndex + 1)
        {
            // Store event position.
            chSyncPositionList.Add(eventPosition);
            logText = "store:" + eventPosition.ToString();
        }
        else
        {
            if (Mathf.Abs((float)(eventPosition - chSyncPositionList[syncIndex])) <= allChSampleRate * chSyncToleranceTime)
            {
                // Return stored position.
                result = chSyncPositionList[syncIndex];
                logText = "correction:" + (result - eventPosition).ToString();
            }
            else
            {
                // Update stored position.
                chSyncPositionList[syncIndex] = eventPosition;
                result = eventPosition;
                logText = "update:" + eventPosition.ToString();
            }
        }
        Debug.Log("channelId:" + channelId.ToString() + " / index:" + syncIndex.ToString() + " / " + logText);
        return result;
    }
}
